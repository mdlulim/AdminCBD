                ...
                <div key={index.toString()} className="timeline__item">
                    ...
                    <div className="content">
                        <div className="title margin-bottom-0">
                            {item.subtype} of <strong>{item.currency.code} {item.amount}</strong> on <Moment date={item.created} format="D MMM YYYY" /> from <strong><GetUserById {...item} /></strong> - <a href={'/transactions/transactions/'+item.id}>Details...</a>
                        </div>
                    </div>
                </div>
            ))}
